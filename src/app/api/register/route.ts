...
const registerRateLimiter = rateLimiter({ limit: 20, windowMs: 60 * 1000 });
...
export async function POST(request: Request) {
  try {
    ...
    const promises = regions.map(async (region: keyof typeof REGION_URLS) => {
      ...
      const teslaApiUrl = `${REGION_URLS[region]}/api/1/partner_accounts`;
      ...
      if (!apiResponse.ok) {
        ...
        throw new Error(`API error for region ${region}: ${data.error || data.msg || 'Unknown API error'}`);
      }
    ...
    const formattedResults = results.map(result => {
      if (result.status === 'fulfilled') {
        ...
        // Extract region from the error message if possible, or mark as unknown
